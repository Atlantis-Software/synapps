...
var request = function(event, route) {
  ...
  this.done(function(data) {
    ...
    data = _.cloneDeepWith(data, function(val) {
...
request.prototype.validate = function() {
  ...
  _.keys(inputRules).forEach(function(key) {
    ...
    // Cast boolean
    if (rule.type === 'boolean' && _.isString(data) && (data === 'true' || data === 'false')) {
    ...
    // Cast integer
    if (rule.type === 'integer' && !validationErrors && _.isString(data)) {
      data = parseInt(data);
    }
    // Cast float
    if (rule.type === 'float' && !validationErrors && _.isString(data)) {
  ...
  if (middlewares && _.isArray(middlewares) && middlewares.length) {
    middlewares.forEach(function(middleware, index) {
      var initMiddleware = function(cb) {
        var timeout = setTimeout(function() {
          var err = new Error(util.format('Middleware %s timeout for route `%s`, %s', middleware.mw_name || index, self.request));
    ...
    middlewaresInit.serie().done(function() {
      ...
      if (err.initMiddlewareInternal) {
        ...
        var error = new Error(util.format('route `%s` could not initialize middlewares', self.request));
        error.original = err;
        cb(error);
      }
...
module.exports = function(synapps) {
  return function(event) {
    var route = synapps._router.getRoute(event.data.method, event.data.request);
    var req = new request(event, route);
    if (route === void 0) {
      return;
    }

    var reqType = event.data.type;
    if (event.data.method) {
      reqType += '/' + event.data.method;
    }

    synapps.debug('info', util.format('Request %s START for route `%s`', reqType, req.request));
